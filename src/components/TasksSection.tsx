...
import { ListChecks, CalendarDays, CalendarRange, Plus, Trash2 } from 'lucide-react';
...
export default function TasksSection() {
  ...
  const handleAdd = () => {
    ...
    addTask(title, scope, memberId || undefined);
    setTitle('');
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">נהלו משימות כלליות, לשבוע הקרוב ולחודש הקרוב.</p>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {SCOPE_GROUPS.map(({ id, title: groupTitle, icon }) => (
          <TaskGroup
            ...
            title={groupTitle}
            icon={icon}
            tasks={data.tasks.filter((t) => t.scope === id)}
